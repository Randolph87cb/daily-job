...
7. 抓取阶段默认直接请求 `tasks_print?lang=en`；如果 Python `requests` 在当前 Windows 环境中出现 TLS / SSL 异常，则自动回退到 PowerShell `Invoke-WebRequest`。
8. 如果前两种 HTTP 抓取都失败，再回退到 Playwright。
9. 生成英文题面到 `en/`。
10. 翻译为逐段穿插对照 Markdown 到 `zh-CN/`。
11. 导出每题一个 PDF，例如 `A.pdf`、`B.pdf`。
12. 自动合并整场 Markdown，并导出整场 PDF，例如 `ABC456.pdf`。
13. 完成后检查：
  - `en/` 下是否生成全部英文题面
