...
- 当前保留的主流程只有一条：
  - 默认复用 `browser-session-manager` 管理的 AtCoder `hybrid` 会话
  - 必要时自动打开浏览器完成人工登录，并在关闭窗口时保存会话
  - 登录阶段优先使用完整 profile 通过 Cloudflare
  - 抓取阶段继续读取保存下来的 `statePath`
  - 翻译成逐段中英对照 Markdown
  - 导出每题一个 PDF
...
默认不要求你预先登录 AtCoder。脚本会先检查全局会话；如果没有登录或登录已失效，会自动弹出浏览器让你登录。默认会话模式是 `hybrid`，会同时保存完整 profile 和 `storageState`，比只保留 `storageState` 更适合处理 Cloudflare 之类的校验。
...
5. 用户在浏览器中完成登录和 Cloudflare 校验后关闭窗口。
6. 关闭窗口时自动保存 profile 和 `storageState`，然后回到导出流程继续抓取 `tasks_print?lang=en`。
7. 生成英文题面到 `en/`。
8. 翻译为逐段穿插对照 Markdown 到 `zh-CN/`。
9. 导出每题一个 PDF，例如 `A.pdf`、`B.pdf`。
10. 自动合并整场 Markdown，并导出整场 PDF，例如 `ABC456.pdf`。
11. 完成后检查：
  - `en/` 下是否生成全部英文题面
...
- 如果脚本提示会话缺失或失效并弹出浏览器：
  - 直接在弹出的页面完成 AtCoder 登录。
  - 如果出现 Cloudflare challenge，就在同一个窗口里先完成 challenge。
  - 登录完成后关闭浏览器窗口，让脚本自动保存 profile 和 `storageState`。
  - 关闭后脚本会自动继续，不需要手动再跑一次。
- 如果浏览器窗口关闭后仍提示未登录：
  - 先确认你关闭的是 Playwright 打开的那个窗口。
