...
- 当前保留的主流程只有一条：
  - 复用 `browser-session-manager` 管理的 AtCoder `storageState`
  - 必要时自动打开浏览器完成人工登录，并在关闭窗口时保存会话
  - 抓取 `tasks_print?lang=en`
  - 翻译成逐段中英对照 Markdown
  - 导出每题一个 PDF
...
默认不要求你预先登录 AtCoder。脚本会先检查全局会话；如果没有登录或登录已失效，会自动弹出浏览器让你登录，关闭窗口时保存会话，然后继续导出。
...
3. 尝试用当前 `storageState` 访问 `tasks_print?lang=en`。
4. 如果检测到未登录，则自动打开可见浏览器。
5. 用户在浏览器中完成登录后关闭窗口。
6. 关闭窗口时自动保存会话，并回到导出流程继续抓取。
7. 生成英文题面到 `en/`。
...
- 如果脚本提示会话缺失或失效并弹出浏览器：
  ...
  - 登录完成后关闭浏览器窗口，让脚本自动保存 `storageState`。
