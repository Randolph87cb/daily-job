...
- AtCoder 相关脚本的思考强度统一优先读取 `.env` 中的 `OPENAI_REASONING_EFFORT`；只有配置文件里的 `statement.reasoning_effort`、`editorial_generation.reasoning_effort` 或命令行 `--reasoning-effort` 显式填写时，才允许覆盖这个统一值；留空时默认沿用服务端 / 模型默认值。
